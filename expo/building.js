...
(function () {
  ...
  if (typeof THREE === "undefined" || !THREE.OrbitControls || !THREE.Reflector) {
    ...
    if (box) {
      ...
      box.innerHTML = cn
        ? "<div style='max-width:460px;text-align:center;line-height:1.7'><div style='font-size:34px;margin-bottom:10px'>🎪</div><b>3D 组件没加载成功，展会打不开</b><br>刷新一下页面；要是一直这样，多半是网络或代理把脚本文件拦了。</div>"
        : zh
  ...
  const LANG_KEY = "copilotExpoLang";
  function detectLang() {
  ...
  const UI = {
    zh: {
      brandT1: "M365 Copilot Agent 博覽會",
      brandT2: "Power of Copilot · {z} 大展區 · {a} 個 Agent 攤位",
      btnExterior: "看場館", btnAtrium: "回主展廳", btnBack: "回主展廳",
      sideTitle: "展場平面圖 · {z} 大展區",
      hintText: "🖱️ 主展廳：拖曳環顧·點展區進入　·　展區內：拖曳轉動看板·點卡看展示",
      searchPh: "搜尋 Agent 名稱或關鍵字…",
      noMatch: "沒有符合的 Agent",
      licReq: "需 M365 Copilot 授權", licFree: "免授權即可使用",
      licReqShort: "需授權", licFreeShort: "免授權",
      tabRun: "▶ 現場展示", tabInfo: "痛點 · 上手 · 提示詞", tabEdm: "電子報原文",
      edmNote: "本 Agent 的「一分鐘小教室」電子報原文", edmOpen: "在新分頁開啟 ↗",
      simBadge: "⚠ 展示示意 · 非真實執行結果", replay: "↻ 再看一次",
      ...
      footSim: "以上為<b>展示示意</b>,用於說明這個 Agent 的對話方式與產出型態,非真實執行結果。內容不指涉特定真實企業,不含捏造的具體數據。",
      ...
      rkH: "高", rkM: "中", rkL: "低",
      cardSolves: "解決這些問題", cardSteps: (n) => n + " 步驟即可上手", cardCta: "▶  觀看展示",
    ...
    cn: {
      ...
      brandT2: "Power of Copilot · {z} 大展区 · {a} 个 Agent 展台",
      btnExterior: "看场馆", btnAtrium: "回主展厅", btnBack: "回主展厅",
      sideTitle: "展场平面图 · {z} 大展区",
      hintText: "🖱️ 主展厅：拖动环视·点展区进入　·　展区里：拖动转展板·点卡片看展示",
      searchPh: "搜 Agent 名称或关键词…",
      noMatch: "没有匹配的 Agent",
      licReq: "需 M365 Copilot 许可", licFree: "无需许可即可使用",
      licReqShort: "需许可", licFreeShort: "免许可",
      tabRun: "▶ 现场展示", tabInfo: "痛点 · 上手 · 提示词", tabEdm: "电子报原文",
      edmNote: "这个 Agent 的“一分钟小课堂”电子报原文", edmOpen: "在新标签页打开 ↗",
      simBadge: "⚠ 展示演示 · 非真实运行结果", replay: "↻ 再看一次",
      ...
      footSim: "以上是<b>展示演示</b>，用来说明这个 Agent 怎么对话、产出长什么样，不是真实运行结果。内容不指向任何真实企业，也没有编造具体数据。",
      ...
      rkH: "高", rkM: "中", rkL: "低",
      cardSolves: "专治这些问题", cardSteps: (n) => n + " 步就能上手", cardCta: "▶  观看展示",
    ...
    en: {
      ...
      hintText: "🖱️ Main hall: drag to look around · click a hall to enter　·　Inside: drag to spin panels · click one for the demo",
      ...
      tabRun: "▶ Live demo", tabInfo: "Info & prompts", tabEdm: "Newsletter",
      edmNote: "The original newsletter issue for this agent", edmOpen: "Open in new tab ↗",
      simBadge: "⚠ Illustrative demo · not a real execution", replay: "↻ Replay",
      ...
      footSim: "The above is an <b>illustrative demo</b> showing how this agent converses and what it produces. It is not a real execution, references no specific real company, and contains no fabricated figures.",
      ...
      cardSolves: "Solves these problems", cardSteps: (n) => n + " steps to get started", cardCta: "▶  Watch the demo",
  ...
  scene.fog = new THREE.Fog(0x141c30, 130, 300);
  ...
  // ---------- Lighting: a big indoor hall lit from an overhead rig ----------
  scene.add(new THREE.HemisphereLight(0xdfe9ff, 0x2b3348, 0.5));
  scene.add(new THREE.AmbientLight(0xffffff, 0.16));
  const key = new THREE.DirectionalLight(0xfff6e8, 0.7);
  key.position.set(40, 80, 36);
  key.castShadow = true;
  ...
  rim.position.set(-48, 44, -56);
  scene.add(rim);

  // ---------- Enclosing shell: dusk sky seen through the hall's glazing ----------
  (function () {
    const mat = new THREE.ShaderMaterial({
      side: THREE.BackSide, fog: false,
      uniforms: {
        top: { value: new THREE.Color(0x0b1120) },
        mid: { value: new THREE.Color(0x18233c) },
        bot: { value: new THREE.Color(0x27405e) },
      },
  ...
  // clouds are a park thing — the convention centre keeps the array so the
  // shared animate loop stays identical, but leaves it empty
  const clouds = [];

  // ---------- Venue floor: carpet tiles outside, polished concrete concourse ----------
  (function () {
    const outer = new THREE.Mesh(new THREE.CircleGeometry(230, 72),
      new THREE.MeshStandardMaterial({ color: 0x1b2438, roughness: 0.92, metalness: 0.05 }));
    outer.rotation.x = -Math.PI / 2; outer.position.y = -0.08; outer.receiveShadow = true;
    scene.add(outer);
    // subtle carpet-tile grid beyond the hall
    const grid = new THREE.GridHelper(420, 84, 0x2c3b57, 0x222c42);
  ...
  function signTexture(zone) {
    ...
    // hall name — luminous lettering, softly haloed so it reads inside the dim venue
    fit(zName(zone), 300, 132, SAFE, `${FF()}`);
    ctx.shadowColor = hexToRgba(zone.color, 0.9);
    ctx.shadowBlur = 32;
    ctx.fillStyle = "#ffffff";
    ctx.fillText(zName(zone), W / 2, 128);
    ctx.fillText(zName(zone), W / 2, 128);
    ctx.shadowBlur = 0;
    // thin rule in the hall colour
    ...
    // booth count
    ctx.fillStyle = "rgba(226,236,255,0.94)";
  ...
  function roomHeaderTexture(zone) {
    ...
    // a dark board behind the text, matching the venue's panel system
    ctx.fillStyle = "rgba(16,22,38,0.9)";
    roundRect(ctx, 8, 8, W - 16, H - 16, 22); ctx.fill();
    ...
    // name
    ctx.textAlign = "left";
    ctx.save(); ctx.shadowColor = hexToRgba(zone.color, 0.8); ctx.shadowBlur = 22;
  ...
  // ---------- Venue shell: glazed hall walls + steel truss roof + entrance foyer ----------
  ...
  function buildShellReal() {
    ...
    const HALL_H = 24;
    // BackSide glazing: normals face the centre, so the walls vanish once the
    // camera is inside the hall but still read as a building from outside
    const glassMat = new THREE.MeshStandardMaterial({
      color: 0x30507e, metalness: 0.35, roughness: 0.2,
    ...
    const darkMat = new THREE.MeshStandardMaterial({ color: 0x1e2739, metalness: 0.4, roughness: 0.6 });

    for (let i = 0; i < segCount; i++) {
      const a = i * segAng + Math.PI / 2;
      ...
      const panel = new THREE.Mesh(new THREE.PlaneGeometry(SHELL_R * segAng * 1.02, HALL_H), glassMat);
      panel.position.set(x, HALL_H / 2, z);
      panel.lookAt(0, HALL_H / 2, 0);
      shellAdd(panel);
      // vertical mullion between panels
      const ma = a - segAng / 2;
      const mull = new THREE.Mesh(new THREE.BoxGeometry(0.2, HALL_H, 0.45), steelMat);
      mull.position.set(Math.cos(ma) * SHELL_R, HALL_H / 2, Math.sin(ma) * SHELL_R);
      mull.lookAt(0, HALL_H / 2, 0);
      wallStruct.add(mull);
    }

    // plinth + parapet rings
    const base = new THREE.Mesh(
      new THREE.CylinderGeometry(SHELL_R + 0.5, SHELL_R + 1.0, 1.4, segCount, 1, true), darkMat);
    base.position.y = 0.7;
    shellAdd(base);
    const crown = new THREE.Mesh(new THREE.TorusGeometry(SHELL_R + 0.7, 0.4, 10, segCount), steelMat);
    crown.rotation.x = Math.PI / 2; crown.position.y = HALL_H;
    shellAdd(crown);

    // ---------- Exhibition-hall roof: radial trusses + skylight ring ----------
    const OCULUS = 15;
    const roof = new THREE.Mesh(new THREE.RingGeometry(OCULUS, SHELL_R + 1, segCount),
      new THREE.MeshStandardMaterial({ color: 0x243049, metalness: 0.4, roughness: 0.65,
        side: THREE.DoubleSide }));
    roof.rotation.x = -Math.PI / 2; roof.position.y = HALL_H;
    roof.receiveShadow = true;
    shellAdd(roof);
    // radial box trusses with lattice webbing — the signature look of a convention hall
    const TRUSSES = 14;
    for (let i = 0; i < TRUSSES; i++) {
      const a = (i / TRUSSES) * Math.PI * 2;
      const len = SHELL_R - OCULUS + 1;
      const midR = OCULUS + len / 2;
      const cx = Math.cos(a) * midR, cz = Math.sin(a) * midR;
      // top and bottom chords
      [0, -1.5].forEach((dy) => {
        const chord = new THREE.Mesh(new THREE.BoxGeometry(len, 0.22, 0.22), steelMat);
        chord.position.set(cx, HALL_H - 0.5 + dy, cz);
        chord.rotation.y = -a;
        shellAdd(chord);
      });
      // diagonal webbing between the chords
      const WEB = 9;
      for (let w = 0; w < WEB; w++) {
        const t0 = OCULUS + (w + 0.5) * (len / WEB);
        const d = new THREE.Mesh(new THREE.BoxGeometry(0.12, 1.8, 0.12), steelMat);
        d.position.set(Math.cos(a) * t0, HALL_H - 1.25, Math.sin(a) * t0);
        d.rotation.set(0, -a, w % 2 ? 0.5 : -0.5);
        shellAdd(d);
      }
      // hall lighting rig hanging off the truss
      for (let L = 0; L < 3; L++) {
        const lr = OCULUS + (L + 1) * (len / 4);
        const lamp = new THREE.Mesh(new THREE.CylinderGeometry(0.45, 0.62, 0.5, 10), darkMat);
        lamp.position.set(Math.cos(a) * lr, HALL_H - 3.0, Math.sin(a) * lr);
        shellAdd(lamp);
        const glow = new THREE.Mesh(new THREE.CircleGeometry(0.5, 12),
          new THREE.MeshBasicMaterial({ color: 0xfff3d6 }));
        glow.rotation.x = Math.PI / 2;
        glow.position.set(Math.cos(a) * lr, HALL_H - 3.27, Math.sin(a) * lr);
        shellAdd(glow);
      }
    }
    // central skylight over the concourse
    const sky = new THREE.Mesh(new THREE.CircleGeometry(OCULUS, 48),
      new THREE.MeshBasicMaterial({ color: 0x7fb0ee, transparent: true, opacity: 0.2,
        side: THREE.DoubleSide }));
    sky.rotation.x = Math.PI / 2; sky.position.y = HALL_H - 0.1;
    shellAdd(sky);
    const skyRim = new THREE.Mesh(new THREE.TorusGeometry(OCULUS, 0.3, 10, 60), steelMat);
    skyRim.rotation.x = Math.PI / 2; skyRim.position.y = HALL_H;
    shellAdd(skyRim);

    // ---------- Entrance foyer: canopy, portal frame, registration desks ----------
    const ex = 0, ez = SHELL_R;
    const canopy = new THREE.Mesh(new THREE.BoxGeometry(26, 0.8, 9), darkMat);
    canopy.position.set(ex, 13.6, ez + 3.2); canopy.castShadow = true;
    shellAdd(canopy);
    const canopyLip = new THREE.Mesh(new THREE.BoxGeometry(26.4, 0.35, 0.5),
      new THREE.MeshStandardMaterial({ color: 0x2f6fd0, metalness: 0.4, roughness: 0.4 }));
    canopyLip.position.set(ex, 13.15, ez + 7.6);
    shellAdd(canopyLip);
    [-11.5, 11.5].forEach((px) => {
      const col = new THREE.Mesh(new THREE.BoxGeometry(1.0, 13.6, 1.0), steelMat);
      col.position.set(ex + px, 6.8, ez + 6.6); col.castShadow = true;
      shellAdd(col);
    });
    // entrance floor: carpet runner
    const mat = new THREE.Mesh(new THREE.PlaneGeometry(24, 14),
      new THREE.MeshStandardMaterial({ color: 0x223256, roughness: 0.95 }));
    mat.rotation.x = -Math.PI / 2; mat.position.set(ex, 0.02, ez + 5); mat.receiveShadow = true;
    shellAdd(mat);
    // registration desks flanking the doors
    [-7.5, 7.5].forEach((px) => {
      const desk = new THREE.Mesh(new THREE.BoxGeometry(5.2, 1.15, 1.6),
        new THREE.MeshStandardMaterial({ color: 0xeef2f8, roughness: 0.5 }));
      desk.position.set(ex + px, 0.6, ez + 4.6); desk.castShadow = true;
      shellAdd(desk);
      const top = new THREE.Mesh(new THREE.BoxGeometry(5.5, 0.16, 1.9),
        new THREE.MeshStandardMaterial({ color: 0x2f6fd0, roughness: 0.4, metalness: 0.3 }));
      top.position.set(ex + px, 1.24, ez + 4.6);
      shellAdd(top);
    });

    // big Copilot logo on the fascia (real PNG)
    const logo = new THREE.Mesh(new THREE.PlaneGeometry(7.2, 7.2),
      new THREE.MeshBasicMaterial({ map: logoTex, transparent: true }));
    logo.position.set(ex, 19.6, ez + 0.5);
    shellAdd(logo);
    // venue name across the fascia
    const nameTex = textTexture(T("parkSign"), 1120, 150, `800 76px ${FF()}`, "#eaf2ff", "#2f6fd0");
    const nameP = new THREE.Mesh(new THREE.PlaneGeometry(21, 2.8),
      new THREE.MeshBasicMaterial({ map: nameTex, transparent: true }));
    nameP.position.set(ex, 15.4, ez + 0.45);
    shellAdd(nameP);
    shellGroup.userData.nameP = nameP;
    const eL = new THREE.PointLight(0x9fc4ff, 0.7, 50); eL.position.set(ex, 13, ez + 7); shellAdd(eL);
  }


  // ---------- Concourse floor: polished concrete with inlaid guide bands ----------
  (function () {
    const geo = new THREE.CircleGeometry(SHELL_R - 1.5, 96);
    const floor = new THREE.Mesh(geo,
      new THREE.MeshStandardMaterial({ color: 0x2a3550, roughness: 0.35, metalness: 0.35 }));
    floor.rotation.x = -Math.PI / 2; floor.position.y = 0.01; floor.receiveShadow = true;
    scene.add(floor);
    // concentric wayfinding bands
    for (let r = 16; r <= SHELL_R - 3; r += 11) {
      const ring = new THREE.Mesh(new THREE.RingGeometry(r - 0.12, r + 0.12, 100),
        new THREE.MeshBasicMaterial({ color: 0x3f5f92, transparent: true, opacity: 0.55 }));
  ...
  scene.add(plazaGroup);
  {
    ...
    const titleTex = textTexture(T("plazaWelcome"), 1180, 112, `800 52px ${FF()}`, "#dbe8ff", "#2f6fd0");
  ...
  ZONES.forEach((zone) => {
    ...
    // ---------- Booth build: a modular exhibition stand per hall ----------
    const zc = new THREE.Color(zone.color);
    const carpetMat = new THREE.MeshStandardMaterial({ color: zc.clone().lerp(new THREE.Color(0x0d1524), 0.62),
      roughness: 0.95, metalness: 0 });
    const panelMat = new THREE.MeshStandardMaterial({ color: 0xf2f5fa, roughness: 0.62, metalness: 0.04 });
    const accMat = new THREE.MeshStandardMaterial({ color: zc, roughness: 0.45, metalness: 0.2,
      emissive: zc.clone(), emissiveIntensity: 0.06 });
    const trussMat = new THREE.MeshStandardMaterial({ color: 0x9aa8bf, metalness: 0.78, roughness: 0.28 });
    const darkMat = new THREE.MeshStandardMaterial({ color: 0x1a2338, roughness: 0.6, metalness: 0.3 });

    // carpet tile in the hall colour, with a thin light strip round the edge
    const pad = new THREE.Mesh(new THREE.BoxGeometry(PAV_W, 0.16, PAV_D), carpetMat);
    pad.position.set(0, 0.08, 0); pad.receiveShadow = true;
    g.add(pad);
    const edgeStrip = new THREE.Mesh(new THREE.TorusGeometry(PAV_W * 0.46, 0.08, 8, 44),
      new THREE.MeshBasicMaterial({ color: zone.color }));
    edgeStrip.rotation.x = Math.PI / 2; edgeStrip.position.y = 0.19;
    g.add(edgeStrip);

    // ---------- Aluminium truss frame around the stand ----------
    const BW = PAV_W - 1.0, BD = PAV_D - 1.0, BH = 6.4;
    const corners = [[-BW / 2, -BD / 2], [BW / 2, -BD / 2], [BW / 2, BD / 2], [-BW / 2, BD / 2]];
    corners.forEach(([cx, cz]) => {
      const post = new THREE.Mesh(new THREE.BoxGeometry(0.32, BH, 0.32), trussMat);
      post.position.set(cx, BH / 2, cz); post.castShadow = true;
      g.add(post);
      for (let w = 0; w < 4; w++) {
        const d = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.1, 0.1), trussMat);
        d.position.set(cx, 1.1 + w * 1.6, cz);
        d.rotation.z = w % 2 ? 0.6 : -0.6;
        g.add(d);
      }
    });
    // top rails
    for (let i = 0; i < 4; i++) {
      ...
      const rail = new THREE.Mesh(new THREE.BoxGeometry(len, 0.26, 0.26), trussMat);
      rail.position.set((x0 + x1) / 2, BH, (z0 + z1) / 2);
      rail.rotation.y = -Math.atan2(z1 - z0, x1 - x0);
      g.add(rail);
    }

    // ---------- Back wall: printed graphic panel with a colour header ----------
    const back = new THREE.Mesh(new THREE.BoxGeometry(BW, 5.6, 0.3), panelMat);
    back.position.set(0, 2.9, -BD / 2 + 0.15); back.receiveShadow = true; back.castShadow = true;
    g.add(back);
    const header = new THREE.Mesh(new THREE.BoxGeometry(BW, 1.1, 0.36), accMat);
    header.position.set(0, 5.15, -BD / 2 + 0.16);
    g.add(header);
    // panel seams so the wall reads as modular
    [-0.34, 0.34].forEach((f) => {
      const seam = new THREE.Mesh(new THREE.BoxGeometry(0.07, 4.4, 0.34),
        new THREE.MeshStandardMaterial({ color: 0xd3dbe8, roughness: 0.6 }));
      seam.position.set(f * BW, 2.5, -BD / 2 + 0.17);
      g.add(seam);
    });
    // half-height side wing walls
    [-1, 1].forEach((s) => {
      const wing = new THREE.Mesh(new THREE.BoxGeometry(0.26, 5.6, BD * 0.42), panelMat);
      wing.position.set(s * (BW / 2 - 0.13), 2.9, -BD / 2 + BD * 0.21 + 0.15);
      wing.castShadow = true;
      g.add(wing);
    });

    // ---------- Hanging sign rig above the booth ----------
    const rigY = BH + 3.0;
    [-1, 1].forEach((s) => {
      const drop = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 3.0, 6), trussMat);
      drop.position.set(s * (BW / 2 - 1.2), BH + 1.5, 0);
      g.add(drop);
    });
    // a lightbox ring hanging over the stand — the classic expo wayfinder
    const ring = new THREE.Mesh(new THREE.TorusGeometry(BW * 0.38, 0.34, 12, 40), accMat);
    ring.rotation.x = Math.PI / 2; ring.position.y = rigY;
    g.add(ring);
    const ringGlow = new THREE.Mesh(new THREE.TorusGeometry(BW * 0.38, 0.16, 10, 40),
      new THREE.MeshBasicMaterial({ color: zone.color }));
    ringGlow.rotation.x = Math.PI / 2; ringGlow.position.y = rigY - 0.28;
    g.add(ringGlow);

    // ---------- Booth furniture: counter, stools, demo pods, plants ----------
    const counter = new THREE.Mesh(new THREE.BoxGeometry(BW * 0.42, 1.1, 0.9),
      new THREE.MeshStandardMaterial({ color: 0xeaeff7, roughness: 0.5 }));
    counter.position.set(-BW * 0.24, 0.63, BD / 2 - 1.6); counter.castShadow = true;
    g.add(counter);
    const cTop = new THREE.Mesh(new THREE.BoxGeometry(BW * 0.44, 0.12, 1.05), accMat);
    cTop.position.set(-BW * 0.24, 1.24, BD / 2 - 1.6);
    g.add(cTop);
    // two demo pods with angled screens
    [0.12, 0.34].forEach((f, i) => {
      const pod = new THREE.Mesh(new THREE.CylinderGeometry(0.42, 0.5, 1.05, 12), darkMat);
      pod.position.set(BW * f, 0.6, BD / 2 - 2.4 + i * 0.9); pod.castShadow = true;
      g.add(pod);
      const scr = new THREE.Mesh(new THREE.BoxGeometry(0.95, 0.6, 0.06),
        new THREE.MeshBasicMaterial({ color: zone.color }));
      scr.position.set(BW * f, 1.35, BD / 2 - 2.4 + i * 0.9);
      scr.rotation.x = -0.45;
      g.add(scr);
    });
    // bar stools
    [-0.34, -0.14].forEach((f) => {
      const seat = new THREE.Mesh(new THREE.CylinderGeometry(0.28, 0.28, 0.14, 12),
        new THREE.MeshStandardMaterial({ color: 0x3a4a68, roughness: 0.6 }));
      seat.position.set(BW * f, 0.78, BD / 2 - 2.9);
      g.add(seat);
      const stem = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.1, 0.72, 8), trussMat);
      stem.position.set(BW * f, 0.36, BD / 2 - 2.9);
      g.add(stem);
    });
    // planters flanking the entrance
    [-1, 1].forEach((s) => {
      const pot = new THREE.Mesh(new THREE.CylinderGeometry(0.42, 0.34, 0.6, 12),
        new THREE.MeshStandardMaterial({ color: 0xdfe5ef, roughness: 0.7 }));
      pot.position.set(s * (BW / 2 - 0.8), 0.3, BD / 2 - 0.6);
      g.add(pot);
      const bush = new THREE.Mesh(new THREE.SphereGeometry(0.62, 12, 10),
        new THREE.MeshStandardMaterial({ color: 0x2f7a3f, roughness: 0.85 }));
      bush.position.set(s * (BW / 2 - 0.8), 1.1, BD / 2 - 0.6); bush.castShadow = true;
      g.add(bush);
    });
    // booth spotlights on the header
    for (let i = 0; i < 3; i++) {
      const lx = (i - 1) * (BW * 0.3);
      const can = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.22, 0.34, 10), darkMat);
      can.position.set(lx, 5.9, -BD / 2 + 0.9);
      can.rotation.x = 0.5;
      g.add(can);
    }
  ...
  // ---------- Venue dressing: wayfinding totems, seating pods, cafe tables ----------
  (function () {
    const trussMat = new THREE.MeshStandardMaterial({ color: 0x9aa8bf, metalness: 0.78, roughness: 0.28 });
    const darkMat = new THREE.MeshStandardMaterial({ color: 0x1a2338, roughness: 0.6, metalness: 0.3 });
    const seatMat = new THREE.MeshStandardMaterial({ color: 0x33415e, roughness: 0.72 });

    // freestanding wayfinding totems between the halls, lit in that hall's colour
    for (let i = 0; i < ZONES.length; i++) {
      const a = (i + 0.5) * ((Math.PI * 2) / ZONES.length) - Math.PI / 2 + OFF;
      const r = RADIUS - 9;
      const x = Math.cos(a) * r, z = Math.sin(a) * r;
      const col = ZONES[i].color;
      const base = new THREE.Mesh(new THREE.CylinderGeometry(0.75, 0.9, 0.3, 14), darkMat);
      base.position.set(x, 0.15, z); scene.add(base);
      const totem = new THREE.Mesh(new THREE.BoxGeometry(1.5, 5.4, 0.35), darkMat);
      totem.position.set(x, 2.9, z); totem.rotation.y = -a; totem.castShadow = true;
      scene.add(totem);
      // lit face on each side, pushed just clear of the totem body
      [-1, 1].forEach((s) => {
        const face = new THREE.Mesh(new THREE.PlaneGeometry(1.2, 4.6),
          new THREE.MeshBasicMaterial({ color: col, transparent: true, opacity: 0.85 }));
        face.position.set(x + Math.cos(a) * 0.19 * s, 3.0, z + Math.sin(a) * 0.19 * s);
        face.rotation.y = -a + (s > 0 ? 0 : Math.PI);
        scene.add(face);
      });
    }

    // seating pods and cafe tables ringing the concourse
    for (let i = 0; i < 10; i++) {
      const a = (i / 10) * Math.PI * 2 + 0.3;
      const r = SHELL_R - 9;
      const x = Math.cos(a) * r, z = Math.sin(a) * r;
      // upholstered bench
      const bench = new THREE.Mesh(new THREE.BoxGeometry(3.0, 0.5, 1.0), seatMat);
      bench.position.set(x, 0.32, z); bench.rotation.y = -a; bench.castShadow = true;
      scene.add(bench);
      const cushion = new THREE.Mesh(new THREE.BoxGeometry(3.0, 0.16, 1.0),
        new THREE.MeshStandardMaterial({ color: 0x44557a, roughness: 0.8 }));
      cushion.position.set(x, 0.62, z); cushion.rotation.y = -a;
      scene.add(cushion);

      // cafe table with two stools, offset from the bench
      const ta = a + 0.14;
      const tx = Math.cos(ta) * (r - 4.2), tz = Math.sin(ta) * (r - 4.2);
      const stem = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.16, 1.0, 10), trussMat);
      stem.position.set(tx, 0.5, tz); scene.add(stem);
      const top = new THREE.Mesh(new THREE.CylinderGeometry(0.7, 0.7, 0.1, 18),
        new THREE.MeshStandardMaterial({ color: 0xe7edf6, roughness: 0.5 }));
      top.position.set(tx, 1.03, tz); top.castShadow = true; scene.add(top);
      [0, Math.PI].forEach((o) => {
        const sx = tx + Math.cos(ta + o) * 1.35, sz = tz + Math.sin(ta + o) * 1.35;
        const st = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.3, 0.12, 12), seatMat);
        st.position.set(sx, 0.62, sz); scene.add(st);
        const sp = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.1, 0.56, 8), trussMat);
        sp.position.set(sx, 0.3, sz); scene.add(sp);
      });

      // floor-standing uplight
      const la = a - 0.2;
      const lx = Math.cos(la) * (r + 3.0), lz = Math.sin(la) * (r + 3.0);
      const post = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.16, 3.6, 8), darkMat);
      post.position.set(lx, 1.8, lz); scene.add(post);
      const head = new THREE.Mesh(new THREE.CylinderGeometry(0.34, 0.2, 0.4, 12),
        new THREE.MeshBasicMaterial({ color: 0xdfeaff }));
      head.position.set(lx, 3.75, lz); scene.add(head);
    }
  })();
  ...
  function buildRoom(zone) {
    ...
    // ---------- Hall interior: a private meeting suite behind the stand ----------
    const zc = new THREE.Color(zone.color);
    // walls are a deep neutral so the agent cards stay the brightest thing in the room
    const wallTone = zc.clone().lerp(new THREE.Color(0x0c1220), 0.84);

    // 1) the venue seen beyond the glazing
    ...
    grad.addColorStop(0.55, "#18233c");
    grad.addColorStop(0.72, "#27405e");
    grad.addColorStop(0.74, "#1b2438");
    grad.addColorStop(1.0, "#141c2e");
    ...
    grp.add(dome);
    const outFloor = new THREE.Mesh(new THREE.CircleGeometry(140, 60),
      new THREE.MeshStandardMaterial({ color: 0x1b2438, roughness: 0.92 }));
    outFloor.rotation.x = -Math.PI / 2; outFloor.position.y = -0.2;
    grp.add(outFloor);

    // carpeted floor with a colour runner
    const floor = new THREE.Mesh(new THREE.BoxGeometry(RW, 0.3, RD),
      new THREE.MeshStandardMaterial({ color: 0x232e46, roughness: 0.94 }));
    ...
    // modular wall panels with aluminium seams — same system as the booth outside
    const panelW = 4.5;
    function panelWall(width, cx, cz, along) {
      const n = Math.round(width / panelW);
      for (let i = 0; i < n; i++) {
        const off = -width / 2 + (i + 0.5) * (width / n);
        const panel = new THREE.Mesh(
          new THREE.BoxGeometry(along === "x" ? width / n - 0.1 : 0.4, RH, along === "x" ? 0.4 : width / n - 0.1),
          new THREE.MeshStandardMaterial({ color: wallTone, roughness: 0.82, metalness: 0.06 })
        );
        if (along === "x") panel.position.set(cx + off, RH / 2, cz);
        else panel.position.set(cx, RH / 2, cz + off);
        panel.receiveShadow = true;
        grp.add(panel);
        // seam post between panels
        const seam = new THREE.Mesh(new THREE.BoxGeometry(0.12, RH, 0.12),
          new THREE.MeshStandardMaterial({ color: 0x8a97ad, metalness: 0.7, roughness: 0.35 }));
        const so = off + (width / n) / 2;
        if (along === "x") seam.position.set(cx + so, RH / 2, cz);
        else seam.position.set(cx, RH / 2, cz + so);
        grp.add(seam);
      }
      // continuous light cove where the wall meets the ceiling
      const cove = new THREE.Mesh(
        new THREE.BoxGeometry(along === "x" ? width : 0.3, 0.22, along === "x" ? 0.3 : width),
        new THREE.MeshBasicMaterial({ color: zone.color, transparent: true, opacity: 0.75 })
      );
      cove.position.set(cx, RH - 0.9, cz);
      grp.add(cove);
    }
    panelWall(RW, 0, -RD / 2, "x");
    panelWall(RW, 0, RD / 2, "x");
    panelWall(RD, -RW / 2, 0, "z");
    panelWall(RD, RW / 2, 0, "z");

    // suspended ceiling grid with recessed lights
    const ceil = new THREE.Mesh(new THREE.BoxGeometry(RW, 0.3, RD),
      new THREE.MeshStandardMaterial({ color: 0x1a2337, roughness: 0.85 }));
    ceil.position.y = RH;
    grp.add(ceil);
    const gridMat = new THREE.MeshStandardMaterial({ color: 0x8a97ad, metalness: 0.65, roughness: 0.4 });
    for (let x = -RW / 2 + 6; x < RW / 2; x += 6) {
      const b = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.14, RD), gridMat);
      b.position.set(x, RH - 0.25, 0); grp.add(b);
    }
    for (let z = -RD / 2 + 6; z < RD / 2; z += 6) {
      const b = new THREE.Mesh(new THREE.BoxGeometry(RW, 0.14, 0.14), gridMat);
      b.position.set(0, RH - 0.25, z); grp.add(b);
      // recessed troffers along each run
      for (let x = -RW / 2 + 9; x < RW / 2 - 3; x += 12) {
        const t = new THREE.Mesh(new THREE.PlaneGeometry(3.4, 0.6),
          new THREE.MeshBasicMaterial({ color: 0xdfeaff }));
        t.rotation.x = Math.PI / 2; t.position.set(x, RH - 0.36, z);
        grp.add(t);
      }
    }

    // back-wall graphic header + accent band
    ...
    // lighting: the scene's rig already reaches here, so add only a soft fill
    // (stacking another hemisphere light washes the suite out to near-white)
    const kl = new THREE.DirectionalLight(0xe8f0ff, 0.35);
    kl.position.set(12, 30, 26); grp.add(kl); grp.add(kl.target);
    const cl = new THREE.PointLight(0xcfe0ff, 0.3, 80); cl.position.set(0, RH - 2, 2); grp.add(cl);
  ...
  function retextureScene() {
    ...
    if (shellGroup.userData.nameP) {
      ...
      m.map.dispose();
      m.map = textTexture(T("parkSign"), 1120, 150, `800 76px ${FF()}`, "#eaf2ff", "#2f6fd0");
      m.needsUpdate = true;
    }
    if (plazaGroup.userData.title) {
      const m = plazaGroup.userData.title.material;
      m.map.dispose();
      m.map = textTexture(T("plazaWelcome"), 1180, 112, `800 52px ${FF()}`, "#dbe8ff", "#2f6fd0");
      m.needsUpdate = true;
    }
