...
(function () {
  ...
  const UI = {
    ...
    cn: {
      brandT1: "M365 Copilot Agent 游乐园",
      brandT2: "Power of Copilot · {z} 大园区 · {a} 项 AI 设施",
      btnExterior: "逛园区", btnAtrium: "回中心广场", btnBack: "回中心广场",
      btnExteriorShort: "逛园区", btnAtriumShort: "回广场",
      sideTitle: "园区导览 · {z} 大主题区",
      hintText: "🖱️ 广场：拖动环视·点园区进场　·　园区里：拖动转设施·点卡片开玩",
      searchPh: "搜 Agent 名称或关键词…",
      noMatch: "没有匹配的 Agent",
      licReq: "需 M365 Copilot 许可", licFree: "无需许可即可使用",
      licReqShort: "需许可", licFreeShort: "免许可",
      tabRun: "▶ 先玩一把", tabInfo: "痛点 · 上手 · 提示词", tabEdm: "电子报原文",
      edmNote: "这个 Agent 的“一分钟小课堂”电子报原文", edmOpen: "在新标签页打开 ↗",
      simBadge: "⚠ 演示效果 · 非真实运行结果", replay: "↻ 再来一次",
      ...
      footSim: "以上是<b>演示效果</b>，用来说明这个 Agent 怎么对话、产出长什么样，不是真实运行结果。内容不指向任何真实企业，也没有编造具体数据。",
      ...
      rkH: "高", rkM: "中", rkL: "低",
      cardSolves: "专治这些问题", cardSteps: (n) => n + " 步就能上手", cardCta: "▶  开玩",
      pavEnter: "点击进场 ▸", agentsSuffix: "设施", backSign: "← 回中心广场",
      plazaWelcome: "欢迎来到 Copilot Agent 游乐园", parkSign: "COPILOT AGENT 游乐园",
      deckSub: (n) => n + " 项 AI 设施", enterExp: "点这里进场",
      loadingZone: "正在打开园区", loadingPlaza: "正在返回中心广场",
    },
    en: {
  ...
  function buildShellReal() {
    ...
    const nameTex = textTexture(T("parkSign"), 1120, 150, `800 76px ${FF()}`, "#ffffff", "#ff5d73");
  ...
  scene.add(plazaGroup);
  {
    ...
    const titleTex = textTexture(T("plazaWelcome"), 1180, 112, `800 52px ${FF()}`, "#7a4a20", "#ffd88a");
  ...
  function applyStaticText() {
    ...
    document.title = isEN()
      ...
      : (isCN() ? "M365 Copilot Agent — 游乐园" : "M365 Copilot Agent — 遊樂園");
  }
  ...
  function retextureScene() {
    ...
    if (shellGroup.userData.nameP) {
      ...
      m.map.dispose();
      m.map = textTexture(T("parkSign"), 1120, 150, `800 76px ${FF()}`, "#ffffff", "#ff5d73");
      m.needsUpdate = true;
    }
    if (plazaGroup.userData.title) {
      const m = plazaGroup.userData.title.material;
      m.map.dispose();
      m.map = textTexture(T("plazaWelcome"), 1180, 112, `800 52px ${FF()}`, "#7a4a20", "#ffd88a");
      m.needsUpdate = true;
    }
