    ...
    turns: {
      zh: [
        ...
        { ask: "責任上限和資料保護要特別看,另外幫我確認有沒有漏掉的標準條款。",
          ...
          steps: ["解析合約結構與條款編號", "逐節比對常見標準條款清單"] },
        { ask: "好,最後也給我一份可以直接改的檔案。",
          steps: ["標定風險等級與建議修改方向", "產出加註解的 Word 檔"] }
      ],
      cn: [
        {
      ...
      en: [
        ...
        { ask: "Liability caps and data protection especially. Also tell me what standard clauses are missing.",
          ...
          steps: ["Parsing contract structure and clause numbering", "Checking each section against the standard clause list"] },
        { ask: "And give me a file I can edit directly.",
          steps: ["Assigning risk levels and suggested redlines", "Producing the annotated Word file"] }
      ]
    },
    title: { zh: "合約審閱意見彙整", cn: "合同审阅意见汇总", en: "Contract Review Summary" },
    data: {
      zh: {
        head: ["條款", "發現的問題", "建議修改", "風險"],
        rows: [
          ["責任上限", "未設上限,理論上責任無限", "改為以合約總價金為上限", "h"],
          ["保密義務", "全份合約未見保密條款", "補入雙向保密條款並約定存續期間", "h"],
          ["交付期限", "僅寫「合理期間內完成」", "改為明確工作日數並定義起算日", "m"],
          ["資料保護", "有提及個資,未寫委外處理與刪除義務", "補入資料處理範圍、再委外限制與終止後刪除", "m"],
          ["終止條款", "僅對方可提前終止", "改為雙方對等,並約定通知期間", "m"],
          ["準據法與爭議解決", "未約定管轄法院", "補入準據法與第一審管轄法院", "l"]
        ]
      },
      cn: {
      ...
      en: {
        head: ["Clause", "Issue found", "Suggested change", "Risk"],
        rows: [
          ["Limitation of liability", "No cap at all — exposure is unbounded", "Cap at total contract value", "h"],
          ["Confidentiality", "No confidentiality clause anywhere in the draft", "Add mutual confidentiality with a survival period", "h"],
          ["Delivery deadline", "Only says \"within a reasonable period\"", "State business days and define the start date", "m"],
          ["Data protection", "Mentions personal data, silent on processing and deletion", "Add processing scope, sub-processor limits, deletion on termination", "m"],
          ["Termination", "Only the counterparty may terminate early", "Make it mutual with a defined notice period", "m"],
          ["Governing law & disputes", "No venue agreed", "Add governing law and court of first instance", "l"]
        ]
      }
    },
    rec: {
